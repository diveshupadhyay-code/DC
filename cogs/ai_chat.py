...
class AIChat(commands.Cog):
    ...
    async def aiserver(self, ctx, status: str = None):
        ...
        current = gs.get("ai_enabled", True)
        ...
        if not status:
            ...
            embed.add_field(
                name="Commands",
                value="`,aiserver on` — enable AI\n`,aiserver off` — disable AI",
                inline=False
            )
            if not is_prem:
                embed.set_footer(text="AI chat requires Happy Premium.")
    ...
    async def handle_message(self, message: discord.Message, sessions: dict, timeout: int):
        ...
        system = (
            ...
            f"Current Time: {readable_time} IST"
        )
