...
class Aliases(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        new_ctx = await self.bot.get_context(fake)
        if new_ctx.valid:
    ...
    async def alias(self, ctx):
        docs = await cmd_aliases_col.find({"guild_id": str(ctx.guild.id)}).to_list(50)
        if not docs:
            embed = discord.Embed(
    ...
    async def alias_add(self, ctx, alias_name: str = None, *, command_name: str = None):
        ...
        if len(alias_name) > 30:
            return await ctx.reply("Alias name must be 30 characters or fewer.")
    ...
    async def alias_remove(self, ctx, alias_name: str = None):
        ...
        if result.deleted_count:
            await ctx.reply(embed=discord.Embed(
                description=f"Alias `{alias_name}` removed.",
                color=0x2B2D31
            ))
        else:
            await ctx.reply(
